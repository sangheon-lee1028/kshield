...
/* ═══════════════════════════════════════════════════════════════════════════
 * Portable BPF helper declarations — no system libbpf-dev required.
 *
 * We claim __BPF_HELPERS_H here so that any later
 *   #include <bpf/bpf_helpers.h>
 * (from kprobe.bpf.c or bpf_tracing.h's internal include) becomes a no-op
 * via bpf_helpers.h's own include guard — eliminating redefinition conflicts
 * no matter which version of libbpf is installed on the host.
 *
 * kernel_defs.h is always #included BEFORE bpf_helpers.h in kprobe.bpf.c,
 * so this block runs first.  All macros and function pointers that were
 * previously provided by bpf_helpers.h are declared here instead.
 *
 * Helper IDs: include/uapi/linux/bpf.h enum bpf_func_id (stable ABI,
 *             unchanged since Linux 5.10 for the helpers used here).
 * ═══════════════════════════════════════════════════════════════════════════ */
#ifndef __BPF_HELPERS_H
#define __BPF_HELPERS_H

/* Kernel UAPI BPF constants (BPF_MAP_TYPE_*, BPF_PROG_TYPE_*, …) */
#include <linux/bpf.h>
...
/* perf_event_output flag: use current CPU */
#define BPF_F_CURRENT_CPU 0xffffffffULL

/* ── BPF helper function-pointer declarations ──────────────────────────── *
 * Each pointer is initialised to its kernel-assigned slot number.          *
 * clang -target bpf rewrites these initialisers into BPF_CALL instructions *
 * at compile time, so the integer cast is safe and intentional.            */
...
static void *(*bpf_get_current_task)(void)
    = (void *)35;
static long  (*bpf_send_signal)(unsigned int sig)
    = (void *)109;
...
static long  (*bpf_probe_read_kernel_str)(void *dst, __u32 size,
                                          const void *unsafe_ptr)
    = (void *)115;
static long  (*bpf_send_signal_thread)(unsigned int sig)
    = (void *)117;

/* bpf_printk — wraps bpf_trace_printk with a compile-time-constant format */
#ifndef bpf_printk
